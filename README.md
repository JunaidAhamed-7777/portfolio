```txt
portfolio/
├── index.html
├── styles/
│   ├── main.css         (layout, sidebar, typography)
│   └── themes.css       (dark/light custom properties)
├── scripts/
│   ├── main.js          (DOM ready, theme toggle, hover descriptions)
│   └── animations.js    (baroque text wave logic)
└── assets/
    └── (future baroque pattern files)
```

- All images taken from flaticon.com. Sources:
    - [sun.png](https://www.flaticon.com/free-icon/sun_10370228)
    - [moon.png](https://www.flaticon.com/free-icon/sleeping_2829162)
    - [special.png](https://www.flaticon.com/free-icon/block_705977?term=mystery&page=1&position=17&origin=search&related_id=705977)
    - [index.png](https://www.flaticon.com/free-icon/rat_616569?term=rat&page=1&position=1&origin=search&related_id=616569)